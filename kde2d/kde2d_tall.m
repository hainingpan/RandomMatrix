function [bandwidth,density,X,Y]=kde2d_tall(data,n,MIN_XY,MAX_XY)
...
N=gather(size(data,1));
if nargin<3
    MAX=max(data,[],1); MIN=min(data,[],1); Range=MAX-MIN;
    MAX_XY=MAX+Range/2; MIN_XY=MIN-Range/2;
end
scaling=MAX_XY-MIN_XY;
if N<=gather(size(data,2))
...
%     [dum,bins(:,i)] = histc(data(:,i),[0:1/M:1],1);
    bincenter=[0:1/M:1];
    dbin=diff(bincenter)/2;
    binedges=[bincenter(1)-dbin(1),bincenter(1:end-1)+dbin,bincenter(end)+dbin(end)];
    [dum,~,bins(:,i)] = histcounts(data(:,i),binedges);
    bins(:,i) = min(bins(:,i),M);
end
